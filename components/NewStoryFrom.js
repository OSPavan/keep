...
const NewStoryFrom = ({setStory,setShowForm,selectedStory,setSelectedStory}) => {
  ...
  return (
    <div className="form">
                    <div className="title">New Keep<br /></div>
                    <input type="text" placeholder="Name"
                        value={name}
                        onChange={(e) => setName(e.target.value)} className="input1" />
                    ...
                    <button className="button-confirm" onClick={addStory} disabled={name.length==0}>Create KEEP →</button>
                </div>
  )
}

export default NewStoryFrom